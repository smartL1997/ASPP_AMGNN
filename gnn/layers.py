...
from gnn.sparse_softmax import Sparsemax
...
class GCN(MessagePassing):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        ...
        return self.propagate(edge_index, x=x, norm=norm), edge_weight.view(-1)
...
class HGPSLPool(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_attr, batch=None):
        ...
        if self.sample:
            ...
            adj = torch.zeros((x.size(0), x.size(0)), dtype=torch.double, device=x.device)
            adj[row, col] = weights
